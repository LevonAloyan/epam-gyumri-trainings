...
import dtoannotations.validationexceptions.DiscountMinimumValueException;

import java.lang.reflect.Field;

public class MinValidationProcess {
            ...
            if (declaredField.isAnnotationPresent(Min.class)) {
                ...
                if (fieldValue instanceof Integer) {
                    ...
                    if (fieldMinimumValue < min.minValue()) {
                        try {
                            throw new DiscountMinimumValueException("\"" + declaredField.getName() + "\" " + "Field has an error, " +
                                    "discount can't be less than " + min.minValue());
                        } catch (DiscountMinimumValueException e) {
                            e.printStackTrace();
